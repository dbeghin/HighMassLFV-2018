...
map<TString, float> GetWeightSys(int CR_number, int n_vert, TLorentzVector tau_p4, int tau_DM, float ratio, TLorentzVector mu_p4, TString lepton, TString mu_lepton, int mu_charge, float top_pt_1, float top_pt_2, float Mll, vector<double> w_pref, TString process, TString mc_nickname) {
  ...
  for (unsigned int i = 0; i<systematics.size(); ++i) {
    ...
    simple_weights[key_nom] = GeneralWeightFunction(CR_number,systematics[i],n_vert,tau_p4,ratio,mu_p4,lepton,top_pt_1,top_pt_2,Mll,w_pref,process,mc_nickname,"nom");
    ...
    for (unsigned int j = 0; j<variations.size(); ++j) {
      ...
      simple_weights[key] = GeneralWeightFunction(CR_number,systematics[i],n_vert,tau_p4,ratio,mu_p4,lepton,top_pt_1,top_pt_2,Mll,w_pref,process,mc_nickname,variations[j]);
    }

  }
  cout << endl << nom_weight << endl << endl << endl;
