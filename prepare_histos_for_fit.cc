...
int main(int argc, char** argv) {
  ...
  else if (binning == "1GeV") bCustom = false;
  ...
  TString nnname_out;
  if (bCustom) nnname_out = "mutauHistos_forLimitSetting_2018.root";
  else nnname_out = "mutauHistos_forLimitSetting_2018_MI.root";
  ...
  for (unsigned int j=0; j<in_names.size(); ++j) {
    for (unsigned int k=0; k<in_sys.size(); ++k) {
      TString name_in = in_sys[k]+"/"+in_names[j]+"ev_Mcol_MtHigh";
