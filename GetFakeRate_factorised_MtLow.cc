...
int main(/*int argc, char** argv*/) {
  //TFile* file_out = new TFile("HighMassLFVMuTau/fakerate_factorised_SSMtLow.root", "RECREATE");
  TFile* file_out = new TFile("HighMassLFVMuTau/fakerate_factorised_OSMtLow.root", "RECREATE");
  TFile* file_in  = new TFile("Figures/histos_fakerate_flavorMtLow.root", "R");
  ...
  vector<TH2D*> h[names.size()][vars.size()][DM.size()];
  for (unsigned int j=0; j<names.size(); ++j) {
    for (unsigned int k=0; k<vars.size(); ++k) { 
      for (unsigned int l=0; l<DM.size(); ++l) {
	for (unsigned int m=0; m<eta.size(); ++m) {
	  //TString name_in = names[j]+vars[k]+"_"+DM[l]+"_MtLow_SS_"+eta[m];
	  TString name_in = names[j]+vars[k]+"_"+DM[l]+"_MtLow_OS_"+eta[m];
	  h[j][k][l].push_back( (TH2D*) file_in->Get(name_in) );
	  h[j][k][l][m]->SetName(names[j]+vars[k]+DM[l]+"_"+eta[m]);
	}
      }
    }
  }
  ...
  for (unsigned int k=0; k<vars.size(); ++k) {
    for (unsigned int l=0; l<DM.size(); ++l) {
      for (unsigned int m=0; m<eta.size(); ++m) {
	TString name_in = vars[k]+"_"+DM[l]+"_"+eta[m];
  ...
  for (unsigned int k=0; k<vars.size(); ++k) {
	...
	cout << hpass_taupt_data[half_k][i]->GetName() << " " << bin_content << endl;
	...
	if (k%2 == 0) hpass_taupt_data_total[half_k] = (TH1D*) hpass_taupt_data[half_k][i]->Clone("FakeRateByTauPt_total_nominal");
	if (k%2 != 0) hfail_ratio_data_total[half_k] = (TH1D*) hfail_ratio_data[half_k][i]->Clone("den3_total");
	if (k%2 == 0) hpass_ratio_data_total[half_k] = (TH1D*) hpass_ratio_data[half_k][i]->Clone("RatioCorrectionFactor_total_nominal");
	...
	if (k%2 == 0) integral_pass_total[half_k] += hpass_taupt_data[half_k][i]->Integral();
      ...
      if (l==0) {
	if (k%2 != 0) hfail_taupt_data_eta[half_k][m] = (TH1D*) hfail_taupt_data[half_k][i]->Clone("den2_total_"+eta[m]);
	if (k%2 == 0) hpass_taupt_data_eta[half_k][m] = (TH1D*) hpass_taupt_data[half_k][i]->Clone("FakeRateByTauPt_nominal_"+eta[m]);
	if (k%2 != 0) hfail_ratio_data_eta[half_k][m] = (TH1D*) hfail_ratio_data[half_k][i]->Clone("den3_total_"+eta[m]);
	if (k%2 == 0) hpass_ratio_data_eta[half_k][m] = (TH1D*) hpass_ratio_data[half_k][i]->Clone("RatioCorrectionFactor_nominal_"+eta[m]);
