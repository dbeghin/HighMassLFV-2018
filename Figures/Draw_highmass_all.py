...
def add_lumi():
    ...
    lumi.SetTextFont (   42 )
    lumi.AddText("2016-8, 135 fb^{-1} (13 TeV)")
    return lumi
...
file=ROOT.TFile("histoStack.root","R")
...
for k in range (0,nvar):
    for l in range (0,len(Mth)):
        ...
        sq_combined_error["up"]=file.Get("square_errors_up_"+var_in)
        sq_combined_error["down"]=file.Get("square_errors_down_"+var_in)

        for signal in signal_names.keys():
            signal_mapp[signal]=file.Get(signal_names[signal]+"_"+var_in)
        
        
        make_plot(var[k]+Mth[l], Data, DY, TT, VV, ST, Faketau, sq_combined_error, signal_mapp)
