...
int main(/*int argc, char** argv*/) {
  ...
  for (unsigned int i=0; i<systs.size(); ++i) {
	...
	for (unsigned int l=0; l<Mth.size(); ++l) {
	  h[i][j][k].push_back( (TH1F*) file_in->Get(systs[i]+"/"+names[j]+vars[k]+Mth[l]) );
	  //cout << systs[i]+"/"+names[j]+vars[k]+Mth[l] << endl;
	  ...
	  if (simpleRebin[k]) {
		...
		if (h[i][j][k][l]->GetBinCenter(iBin) < rebin_array[jBin-1]) continue;
